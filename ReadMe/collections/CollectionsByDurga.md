...
- Boolean addAll(Collection c); // c can be arraylist, set, collectionsExample.stack
...
```
- **NOTE** : Every collection class implements Cloneable and Serializable Interface. Only collectionsExample.vector and Arraylist implements RandomAccess Interface
...
```
- **Q : How to make collectionsExample.hashMap and HashSet as Synchronized ?**
```
...
- Most of the methods present in collectionsExample.vector are synchronized. Hence Vector Object is Thread-Safe
- Best Choice if the Frequent operation is Retrieval

## Methods
- **For adding**
- add(Object o)  -> method in collection , can be used in Vector
- add(int index, Object o)  -> method of collection, can be used in collectionsExample.vector
...
```
    ...
    Object elementAt(int index);  // from collectionsExample.vector
    Object firstElement(); // from collectionsExample.vector
    Object lastElement(); // from collectionsExample.vector
```
## Other methods
```
...
- Create an empty collectionsExample.vector Object with specified initial Capacity
- 3.Vector v = new Vector(int initialCapacity, int incrementalCapacity);
- 4.Vector v = new Vector(Collection c);
- Creates an equivalent Vector Object for the given collection
## Difference Between Arraylist and Vector
- Arraylist is non synchronized but collectionsExample.vector is synchronized,
- ArrayList is not thread safety but Vector is thread Safety
- Performance is relatively high as compared to collectionsExample.vector, performance wise collectionsExample.vector is not up to the mark
- ArrayList came in 1.2 version, that's why its not legacy class, but collectionsExample.vector is a legacy class
...
```
    Stack collectionsExample.stack = new Stack();
    Methods
...
- V is any collectionsExample.vector
...
```
   ...
   str = itr.next();
           ...
           */
   `System.out.println("Key: "+str+" & Value: "+collectionsExample.hashtable.get(str));
    }
```
## Set
...
- If we want to represent a group of individual object as a single Entity where duplicates are not allowed but all objects should be inserted according to some sorting order then we should go for sortedSet(I)
...
## TreeSet
- The underlying data structure for collectionsExample.treeSet is Balanced Tree
...
- Q. how to sort below collectionsExample.treeSet
```
    TreeSet t = new TreeSet();
    t.add(new StringBuffer(“a”))
    t.add(new StringBuffer(“A”))
